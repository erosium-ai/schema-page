...
/** Stored snapshot so the public page can render the badge without re-calling ABR every visit. */
...
function parsePossiblyWrappedJson(text: string): Record<string, unknown> | null {
  ...
  if (wrapped?.[1]) return JSON.parse(wrapped[1]) as Record<string, unknown>;
  return null;
}
